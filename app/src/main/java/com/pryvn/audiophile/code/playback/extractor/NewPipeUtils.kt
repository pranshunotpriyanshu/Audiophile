...
class NewPipeDownloaderImpl(
    ...
    private fun okhttp3.Response.toNewPipeResponse(): Response {
        ...
        return Response(code, message, headers.toMultimap(), rawBytes.toString(Charsets.UTF_8), rawBytes, request.url.toString())
    }
...
class NewPipeUtils(downloader: Downloader) {
    init { NewPipe.init(downloader) }
